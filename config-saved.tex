...
\newcommand{\fairemarges}{
    ...
    \fancyfoot[R]{2025-2026} % BOTTOM RIGHT
    \fancyhead[L]{\nouppercase{\leftmark}} % TOP LEFT
    \fancyhead[R]{} % TOP RIGHT
    \pagestyle{fancy}
}

% Nouvelle commande pour les chapitres non numérotés avec en-tête personnalisé
\newcommand{\chapterstar}[1]{%
    \chapter*{#1}%
    \markboth{#1}{#1}% Définit l'en-tête pour ce chapitre non numéroté
    \addcontentsline{toc}{chapter}{#1}% Ajoute au sommaire (optionnel)
}
...
\ThisLRCornerWallPaper{1}{logos/background.png}
    ...
    %\hspace{-2cm}
    \includegraphics[width=0.3\textwidth]{logos/logo_dschang.png}\hfill
	\includegraphics[width=0.3\textwidth]{logos/logo_dschang.png}\par
	\vspace{1cm} %Insertion du logo
	{\scshape\LARGE Université de Dschang - Faculté des Sciences \par} %Nom de l'université
	...
	\begin{minipage}{0.5\textwidth} % Minipage pour faire deux colonnes
            ...
            \textbf{Élèves :}\par % Pour le titre au dessus des noms à gauche
                \eleves \par
            \end{flushleft}
        \end{minipage}
        ~
